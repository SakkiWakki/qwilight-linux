#!/bin/bash
# GUARD: never start while the user's game (or any other Qwilight.exe) runs in the prefix.
if pgrep -x Qwilight.exe >/dev/null; then echo "REFUSED: Qwilight.exe is already running" >&2; exit 3; fi
...
Xvfb :6 -screen 0 2560x1440x24 +extension GLX -nolisten tcp >"$S/xvfb.log" 2>&1 & XPID=$!
sleep 2
...
env DISPLAY=:6 QWILIGHT_DEV_BUILD="${QWILIGHT_DEV_BUILD:-0}" LIBGL_ALWAYS_SOFTWARE=1 __GLX_VENDOR_LIBRARY_NAME=mesa __EGL_VENDOR_LIBRARY_FILENAMES=/usr/share/glvnd/egl_vendor.d/50_mesa.json GALLIUM_DRIVER=llvmpipe \
    QWILIGHT_WINEDEBUG="${QWDEBUG:-warn+module,+timestamp,+seh}" setsid nohup $S/run-betahl waitforexitandrun "$D/Qwilight.exe" >/dev/null 2>&1 < /dev/null & LPID=$!
sleep 5
PID=$(pgrep -x Qwilight.exe -P "$LPID" 2>/dev/null | head -1); [ -z "$PID" ] && for c in $(pgrep -x Qwilight.exe); do [ "$(ps -o sid= -p $c 2>/dev/null | tr -d " ")" = "$LPID" ] && PID=$c; done; echo "pid ${PID:-none} (launcher $LPID, session-scoped) at $(date +%T)"
python3 "$(cd "$(dirname "$0")" && pwd)/xscenario.py" :6 "Qwilight AMD64" "$S/scenario" > "$S/xscenario.log" 2>&1 &
